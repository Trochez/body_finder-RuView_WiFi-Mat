...
fn main()->Result<()> {
    if env::args().any(|a|a=="--help"||a=="-h"){println!("body-finder-replay --input session.jsonl [--output canonical.jsonl]");return Ok(());}
    let input=value("--input").context("--input is required")?;
    let measurements=read_measurements(BufReader::new(File::open(&input).with_context(||format!("open {input}"))?))?;
    if measurements.is_empty(){bail!("no measurements");}
    let digest=deterministic_digest(&measurements)?;
    let canonical=canonical_measurements(measurements);
    if let Some(output)=value("--output") { let mut f=File::create(output)?; for m in &canonical { writeln!(f,"{}",serde_json::to_string(m)?)?; } }
    ...
    println!("{}",serde_json::to_string_pretty(&summary)?); Ok(())
}
