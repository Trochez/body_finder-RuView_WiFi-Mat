...
def main():
 ap=argparse.ArgumentParser();ap.add_argument('exports',nargs='+');ap.add_argument('--detector',required=True);ap.add_argument('--output',default='dev20.5-campaign-report.json');a=ap.parse_args();fail=[];rows=[]
 ...
 o={'schema_version':2,'release':'dev-20.5','count':len(rows),'recall':recall,'specificity':spec,'moving_recall':mr,'stationary_recall':sr,'healthy_indeterminate_rate':ind,'failures':fail,'final_go':not fail,'dev21_blocked':bool(fail),'screenshots_required':False,'human_localization_validated':False,'rescue_use_validated':False}
