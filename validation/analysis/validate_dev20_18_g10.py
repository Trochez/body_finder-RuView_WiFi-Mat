#!/usr/bin/env python3
"""Strict dev-20.18 G10 physical validator. No screenshots are consumed."""
...
def main(argv=None):
    ap=argparse.ArgumentParser(description='Validate exactly six dev-20.18 physical G10 JSONs');ap.add_argument('files',nargs='*');ap.add_argument('--output');ns=ap.parse_args(argv)
    ...
    out={'schema':'G10Dev2018PhysicalValidationV1','release':'dev-20.18','files':len(paths),'unique_nodes':sorted(unique_nodes),'scenarios':{'SMOKE_CAL_EMPTY':empty,'HUMAN_MOVING':human},'minimum_duration_ms':MIN_DURATION_MS,'errors':errs,'g10':'GO' if not errs else 'NO_GO','g10_go':not errs,'g11':'UNBLOCKED' if not errs else 'BLOCKED','dev21':'UNBLOCKED' if not errs else 'BLOCKED','screenshots_required':False}
