...
ALGO='deterministic-multinode-rssi-fusion-v7'
...
def metadata(d):
    return d.get('export_metadata') or {}
...
def day_id(d):
    ...
    return str(m.get('campaign_day') or m.get('day_id') or r.get('campaign_day') or d.get('campaign_day') or '')
...
def replay(detector,p,path,fails):
    ...
    if not isinstance(inp,dict):
        fails.append(f'{path}: canonical replay missing'); return
    q=subprocess.run([detector],input=json.dumps(inp),text=True,capture_output=True)
    if q.returncode:
        fails.append(f'{path}: detector CLI failed ({q.returncode})'); return
    try: off=json.loads(q.stdout)
    except Exception:
        fails.append(f'{path}: detector CLI emitted invalid JSON'); return
    if off.get('canonical_digest')!=p.get('canonical_digest') or off.get('prediction')!=p.get('prediction'):
        fails.append(f'{path}: Android/CLI canonical parity failed')

def main():
    ap=argparse.ArgumentParser()
    ap.add_argument('exports',nargs='+')
    ap.add_argument('--detector',required=True)
    ap.add_argument('--output',default='dev20.7-campaign-go-no-go.json')
    ...
    for path in a.exports:
        ...
        p=presence(d); c=calibration(d,p); sc=scenario(d); day=day_id(d); node=node_id(d)
        r=d.get('validation_run') or {}; dur=int(r.get('elapsed_ms') or r.get('duration_ms') or 0)
        if sc not in SCENARIOS: fails.append(f'{path}: unknown/missing scenario {sc!r}')
        if not day: fails.append(f'{path}: campaign day id missing')
    ...
    days={x['day'] for x in rows if x['day']}
    nodes={x['node'] for x in rows if x['node']}
    ...
    # Every day must contain all 9 scenarios x all 3 devices.
    for day in days:
    ...
    tp=sum(1 for x in rows if x['scenario'] in HUMAN and x['prediction']=='HUMAN_EVIDENCE')
    fn=sum(1 for x in rows if x['scenario'] in HUMAN and x['prediction']!='HUMAN_EVIDENCE')
    tn=sum(1 for x in rows if x['scenario'] in NEG and x['prediction']=='NO_HUMAN_EVIDENCE')
    fp=sum(1 for x in rows if x['scenario'] in NEG and x['prediction']!='NO_HUMAN_EVIDENCE')
    ind=sum(1 for x in rows if x['prediction']=='INDETERMINATE')
    recall=tp/max(1,tp+fn); specificity=tn/max(1,tn+fp); ind_rate=ind/max(1,len(rows))
    moving=[x for x in rows if x['scenario']=='HUMAN_MOVING']; stationary=[x for x in rows if x['scenario']=='HUMAN_STATIONARY_CENTER']
    moving_recall=sum(x['prediction']=='HUMAN_EVIDENCE' for x in moving)/max(1,len(moving))
    stationary_recall=sum(x['prediction']=='HUMAN_EVIDENCE' for x in stationary)/max(1,len(stationary))
    ...
    for node in sorted(nodes):
        rr=[x for x in rows if x['node']==node]
        h=[x for x in rr if x['scenario'] in HUMAN]; n=[x for x in rr if x['scenario'] in NEG]
        per_device[node]={
            'human_recall':sum(x['prediction']=='HUMAN_EVIDENCE' for x in h)/max(1,len(h)),
            'specificity':sum(x['prediction']=='NO_HUMAN_EVIDENCE' for x in n)/max(1,len(n)),
            'indeterminate_rate':sum(x['prediction']=='INDETERMINATE' for x in rr)/max(1,len(rr))
        }
