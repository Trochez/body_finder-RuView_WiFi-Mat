#!/usr/bin/env python3
"""Strict dev-20.18 three-node PRE_RUN validator. JSON only; screenshots are ignored."""
...
def main(argv=None):
    ...
    for p,d in zip(ps,docs):
        ...
        state=s(first(d,'calibration_state','state')).upper();dist=ab(first(d,'distributed_calibration_ready'));promoted=ab(first(d,'local_artifact_promoted'));binding=s(first(d,'current_calibration_binding_digest'))
    ...
    out={'schema':'PreRunDev2018PhysicalValidationV1','release':'dev-20.18','files':len(ps),'unique_nodes':sorted(unique),'errors':errs,'pre_run':'GO' if not errs else 'NO_GO','pre_run_go':not errs,'screenshots_required':False,'next_step':'RUN_G10' if not errs else 'STOP_AND_SHARE_EXACTLY_3_PRE_RUN_JSON'}
