...
from dev20_3_detector import infer_fused_presence, canonical_result, ALGORITHM_VERSION, PARAMETER_HASH
...
def main()->int:
    ...
    out={'schema_version':2,'release':'dev-20.3','algorithm_version':ALGORITHM_VERSION,'detector_parameter_hash':PARAMETER_HASH,'primary_inference_unit':'SYNCHRONIZED_3_NODE_SCENARIO','baseline_regression':'PASS' if not bad else 'FAIL',
      ...
      'peer_authoritative_consistency':'PASS' if all(s.get('peer_authoritative_consistent',True) for s in sessions) else 'FAIL','physical_acceptance':physical,'final_go':bool(a.final_test and not failures),'metrics':m,'scenarios':rows,'failures':failures,'screenshots_required':False,'human_localization_validated':False,'rescue_use_validated':False,'claims_blocked':['localization','rescue_use','proof_of_absence']}
