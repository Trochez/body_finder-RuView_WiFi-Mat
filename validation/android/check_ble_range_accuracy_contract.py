...
APP_JSON=ROOT/'apps/mobile/app.json'; INDEX_TS=ROOT/'apps/mobile/modules/body-finder-native/index.ts'; APP=ROOT/'apps/mobile/App.tsx'; VERSION=ROOT/'apps/mobile/src/version.ts'
...
module=MODULE.read_text(); estimator=ESTIMATOR.read_text(); service=SERVICE.read_text(); manifest=MANIFEST.read_text(); app=APP.read_text(); version=VERSION.read_text(); index_ts=INDEX_TS.read_text(); graph=GRAPH.read_text(); fusion=FUSION.read_text(); fitter=FITTER.read_text(); app_json=json.loads(APP_JSON.read_text()); profiles=json.loads(PROFILE.read_text()); json.loads(PROFILE_SCHEMA.read_text()); p0c=json.loads(P0C.read_text())
...
require('0.2.0-experimental.11' in version,'mobile build is not experimental.11'); require(app_json['expo']['android']['versionCode']==11,'Android versionCode must be 11'); require(app_json['expo']['extra']['releaseIteration']=='experimental.11','releaseIteration must be experimental.11'); require('VALIDATED_COARSE_BLE_METRIC_0P5_TO_5M' in app,'physical-truth classification missing')
...
print(json.dumps({'contract':'experimental.11 preserves validated coarse BLE metric physics','profile_id':active['profile_id'],'rssi_at_1m_dbm':active['rssi_at_1m_dbm'],'path_loss_exponent':active['path_loss_exponent'],'valid_domain_m':[0.5,5.0],'physical_confidence':'COARSE'},indent=2))
