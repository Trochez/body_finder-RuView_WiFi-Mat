...
require('0.2.0-experimental.13' in version,'mobile build is not experimental.13'); require(app_json['expo']['android']['versionCode']==13,'Android versionCode must be 13'); require(app_json['expo']['extra']['releaseIteration']=='experimental.13','releaseIteration must be experimental.13'); require('VALIDATED_COARSE_BLE_METRIC_0P5_TO_5M' in app,'physical-truth classification missing')
...
print(json.dumps({'contract':'experimental.13 preserves validated coarse BLE metric physics','profile_id':active['profile_id'],'rssi_at_1m_dbm':active['rssi_at_1m_dbm'],'path_loss_exponent':active['path_loss_exponent'],'valid_domain_m':[0.5,5.0],'physical_confidence':'COARSE'},indent=2))
