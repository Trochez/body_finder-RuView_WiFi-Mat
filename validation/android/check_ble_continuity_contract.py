...
def main() -> None:
    ...
    require(active["physical_confidence"] == "COARSE", "confidence changed from approved COARSE state")
    ...
    require("rssi_samples_dbm" in module and "JSONArray(fresh.map { it.rssi })" in module, "calibration snapshot does not use valid-only queue")
    require("invalid_rssi_values_exported" in module and "false" in module, "calibration snapshot invalid-RSSI truth flag missing")
    ...
    for token in [
        ...
        "FabricRuntime.lastValidRangeByPeer.remove(peerId)", "OUT_OF_DOMAIN_LOW", "OUT_OF_DOMAIN_HIGH",
    ]:
