...
import { SafeAreaView, View, Text, StyleSheet, Pressable, TextInput, ScrollView, Share, Platform } from 'react-native';
...
type Estimate = { x_m:number; y_m:number; range_m:number; bearing_deg:number; human_confidence:number; uncertainty_percent:number; error_radius_95_m:number; quality:string; method:string; state:string };

const T = {
  en: { title:'Body Finder – RuView', experimental:'EXPERIMENTAL COMMON-DEVICE RF — NOT VALIDATED FOR RESCUE USE', radar:'Radar', expert:'Expert', calibrate:'Calibrate empty scene', scanning:'Start scan', stop:'Stop scan', peers:'nodes', pos:'This node position (meters)', share:'Share result JSON', empty:'Keep the target area empty while calibrating.', noTarget:'No defensible position yet. Need ≥3 calibrated, scanning nodes with known positions and measurable RF disturbance.', uncertainty:'uncertainty', confidence:'human confidence', network:'OPEN / UNTRUSTED FIELD NETWORK', evidence:'Evidence: commodity Wi-Fi RSSI disturbance (not CSI)', set:'Set position' },
  es: { title:'Body Finder – RuView', experimental:'RF DE DISPOSITIVOS COMUNES EXPERIMENTAL — NO VALIDADO PARA USO DE RESCATE', radar:'Radar', expert:'Experto', calibrate:'Calibrar escena vacía', scanning:'Iniciar escaneo', stop:'Detener escaneo', peers:'nodos', pos:'Posición de este nodo (metros)', share:'Compartir resultado JSON', empty:'Mantén vacía el área objetivo durante la calibración.', noTarget:'Aún no hay posición defendible. Se necesitan ≥3 nodos calibrados, escaneando, con posición conocida y perturbación RF medible.', uncertainty:'incertidumbre', confidence:'confianza humana', network:'RED DE CAMPO ABIERTA / NO CONFIABLE', evidence:'Evidencia: perturbación RSSI Wi‑Fi commodity (no CSI)', set:'Guardar posición' }
};

function estimate(nodes: Ad[]): Estimate | null {
  ...
  const quality = uncertainty<=20?'HIGH':uncertainty<=40?'MEDIUM':uncertainty<=70?'LOW':'VERY_LOW';
  return {x_m:x,y_m:y,range_m:range,bearing_deg:Math.atan2(x,y)*180/Math.PI,human_confidence:confidence,uncertainty_percent:uncertainty,error_radius_95_m:error,quality,method:'EXPERIMENTAL_RSSI_DISTURBANCE_CENTROID_V1',state:meanZ>=5?'PROBABLE_HUMAN':'POSSIBLE_HUMAN'};
}
...
export default function App() {
  ...
  useEffect(()=>{
    ...
    const timer = setInterval(()=>{
      if(!live) return;
      try {
        const l = JSON.parse(BodyFinderNative.getLocalAdvertisementJson()) as Ad;
        const p = JSON.parse(BodyFinderNative.getPeersJson()) as Ad[];
        setLocal(l); setPeers(p);
  ...
  const target = useMemo(()=>estimate(nodes),[nodes]);
  ...
  async function calibrate(){
    ...
    try {
      ...
      if(samples.length<8) throw new Error('Not enough live Wi-Fi RSSI samples. Connect this device to Wi-Fi and grant requested permissions.');
  ...
  async function share(){
    const payload={
      report_version:1, generated_at:new Date().toISOString(), app:'Body Finder – RuView', build:'0.1.0-experimental.1',
      truth:'LIVE_DEVICE_CAPABILITIES_AND_RSSI__EXPERIMENTAL_LOCALIZATION_NOT_VALIDATED',
      node_id:nodeId, capabilities:caps, local, peers, coordinator_node_id:coordinator, estimate:target,
      instructions:'Return this JSON plus Ubuntu/Windows JSONL logs to ChatGPT for analysis.'
    };
  ...
  return <SafeAreaView style={s.safe}><StatusBar style="light" />
    ...
    {mode==='radar'?<ScrollView contentContainerStyle={s.body}>
      ...
      <View style={s.radar}>
        {[1,2,3].map(i=><View key={i} style={[s.ring,{width:i*90,height:i*90,borderRadius:i*45}]} />)}
        <View style={s.operator}/>
        {nodes.filter(n=>n.position).map((n,i)=><View key={n.node_id} style={[s.sensor,{left:145+(n.position!.x_m*18),top:145-(n.position!.y_m*18)}]}><Text style={s.dotLabel}>{i+1}</Text></View>)}
        {target&&<><View style={[s.targetRing,{width:Math.min(260,target.error_radius_95_m*36),height:Math.min(260,target.error_radius_95_m*36),borderRadius:130,left:150-Math.min(260,target.error_radius_95_m*36)/2+target.x_m*18,top:150-Math.min(260,target.error_radius_95_m*36)/2-target.y_m*18}]}/><View style={[s.target,{left:144+target.x_m*18,top:144-target.y_m*18}]}/></>}
      </View>
