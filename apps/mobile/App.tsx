...
export default function App() {
  ...
  async function sharePreRunDiagnostic(){try{const fresh=JSON.parse(BodyFinderNative.getDiagnosticsJson());const blocking:string[]=[];if(!authorityStatus?.consensus)blocking.push(...(authorityStatus?.blocking_reasons??['AUTHORITY_NOT_3_OF_3']));if((scenarioStatus?.ack_count??0)!==3)blocking.push(`SCENARIO_ACK_${scenarioStatus?.ack_count??0}_OF_3`);if((calibrationStatus?.peer_ack_count??0)!==3)blocking.push(`CALIBRATION_ACK_${calibrationStatus?.peer_ack_count??0}_OF_3`);if((startBarrier?.ready_count??0)!==3)blocking.push(`RUN_START_READY_${startBarrier?.ready_count??0}_OF_3`);const context={release:'dev-20.19',build:BUILD,evidence_schema:'v21',artifact_build_identity:`dev-20.19:${BUILD}`,generated_at:new Date().toISOString(),session_id:local?.session_id??null,node_id:local?.node_id??null,authority_status:authorityStatus,membership:nodes.map(n=>({node_id:n.node_id,instance_epoch:(n as any).instance_epoch??null,membership_lease_age_ms:(n as any).membership_lease_age_ms??null,membership_lease_state:(n as any).membership_lease_state??null,advertised_authority_view:(n.control_plane as any)?.authority_view_v1??null})),scenario_status:scenarioStatus,calibration_status:calibrationStatus,run_start:startBarrier,freeze_barrier:freezeBarrier,artifact_outbound_state:fresh?.fabric_diagnostics?.wire_transport_v11?.artifact_peer_state_v1??null,artifact_receiver_state:fresh?.fabric_diagnostics?.wire_transport_v11?.artifact_receiver_state_v1??null,wire_transport_telemetry_v13:fresh?.fabric_diagnostics?.wire_transport_v11??null,geometry_summary:{state:geometry?.state??null,dimension:geometry?.dimension??null,positions:geometry?.positions?.length??0,measurement_health:graphDiagnostics.measurement_health},blocking_reasons:[...new Set(blocking)]};const rawSerialized=BodyFinderNative.exportPreRunDiagnosticJson(JSON.stringify(context));const normalized=JSON.parse(rawSerialized);Object.assign(normalized,{release:'dev-20.19',build:BUILD,evidence_schema:'v21',report_version:REPORT_VERSION,snapshot_schema_version:RELEASE.snapshotSchemaVersion,artifact_build_identity:`dev-20.19:${BUILD}`,evidence_contract_version:'dev20.19-state-lifecycle-json-evidence-v21'});normalized.diagnostic_contract={...(normalized.diagnostic_contract??{}),schema:'dev20.19-diagnostic-contract-v21'};normalized.evidence_contract={...(normalized.evidence_contract??{}),schema:'dev20.19-state-lifecycle-json-evidence-v21'};const serialized=JSON.stringify(normalized,null,2);const fn=`pre-run-diagnostic-dev20.19-${String(local?.node_id??'node').slice(-8)}.json`;if(Platform.OS==='android'){if(!BodyFinderNative.shareJsonFile(serialized,fn))throw new Error('Could not share diagnostic JSON')}else await Share.share({message:serialized,title:fn});}catch(c:any){setError(String(c?.message??c));}}
  ...
  async function share() {
    ...
    const payload = {
      report_version: REPORT_VERSION, release: 'dev-20.19', evidence_schema: 'v21', snapshot_schema_version: RELEASE.snapshotSchemaVersion, artifact_build_identity: `dev-20.19:${BUILD}`,
      ...
      evidence_contract: {
        schema: 'dev20.19-state-lifecycle-json-evidence-v21', screenshots_required: false, json_self_contained: true,
      ...
      diagnostic_contract: {...(freshDiagnostics?.diagnostic_contract ?? {}), schema:'dev20.19-diagnostic-contract-v21'},
      ...
      {mode === 'radar' ? (
        <ScrollView contentContainerStyle={s.body}>
          ...
          <View style={s.card}><Text style={s.h2}>Validation run</Text><Text style={s.text}>run: {validationRun?.run_id ?? '—'} · active: {String(Boolean(validationRun?.active))}</Text>
            <Text style={s.text}>elapsed: {validationRun?.elapsed_ms ?? 0} ms · acceptance ≥330s: {String(Boolean(validationRun?.acceptance_duration_eligible))} · frozen: {String(Boolean(validationRun?.snapshot_frozen))} · schema: {validationRun?.snapshot_schema_version ?? RELEASE.snapshotSchemaVersion}</Text>
