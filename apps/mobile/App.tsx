...
const T = {
  en: {
    ...
    noTarget: 'Human scanning remains intentionally blocked until the experimental.13 validation-integrity gate is reviewed. This build validates sensor geometry acquisition continuity only.',
  ...
  es: {
    ...
    noTarget: 'El escaneo humano permanece bloqueado intencionalmente hasta revisar el gate de continuidad de adquisición BLE de experimental.13. Esta build valida únicamente la continuidad de adquisición de la geometría de sensores.',
...
export default function App() {
  ...
  async function share() {
    ...
    const payload = {
      ...
      evidence_contract: {
        schema: 'dev13-self-contained-json-evidence-v2', screenshots_required: false, json_self_contained: true,
      ...
      instructions: 'Return the exported JSON files only. Screenshots are not required for dev-13 evidence. Use >=330 s for acceptance; short runs remain diagnostic only. Do not change calibration, minSamples, freshness, holdover or solver settings. Human scanning remains blocked.',
    };
    await Share.share({ message: JSON.stringify(payload, null, 2), title: 'Body Finder experimental.13 self-contained validation result' });
  }
      ...
      {mode === 'radar' ? (
        ...
        <ScrollView contentContainerStyle={s.body}>
          <View style={s.card}><Text style={s.h2}>Truth / source classification</Text><Text style={s.text}>Build: {BUILD} · protocol 2</Text>
            ...
            <Text style={s.text}>Acquisition: experimental.13 uses manufacturer-filtered LOW_LATENCY scanning as FILTERED_PRIMARY. CALLBACK_TYPE_ALL_MATCHES is only the Android callback setting inside the filtered scan; the no-filter path is bounded UNFILTERED_RECOVERY only.</Text>
            ...
            <Text style={s.text}>Human scanning: BLOCKED until the experimental.13 validation-integrity gate is reviewed.</Text>
