...
class BodyFinderNativeModule : Module() {
  override fun definition() = ModuleDefinition {
    ...
    Function("updatePublishedGeometry") { publish: Boolean, geometryJson: String? ->
      FabricRuntime.publishedGeometryJson = if (publish && !geometryJson.isNullOrBlank()) {
        try {
          JSONObject(geometryJson).toString()
        } catch (_: Throwable) {
          null
        }
      } else {
  ...
  private fun capabilityMap(ctx: Context): JSONObject {
    ...
    val blePerm = Build.VERSION.SDK_INT < 31 ||
      (hasPermission(ctx, Manifest.permission.BLUETOOTH_SCAN) && hasPermission(ctx, Manifest.permission.BLUETOOTH_ADVERTISE))
    return JSONObject().apply {
      ...
      put("ble", when {
        ...
        !blePerm -> probe("PERMISSION_REQUIRED", "Bluetooth scan/advertise permission required")
      ...
      put("udp_fabric", probe("WORKING_DEGRADED", "local UDP multicast/broadcast; verify on the actual LAN"))
  ...
  private fun startBle(ctx: Context) {
    try {
      ...
      if (Build.VERSION.SDK_INT >= 31 && (!hasPermission(ctx, Manifest.permission.BLUETOOTH_SCAN) || !hasPermission(ctx, Manifest.permission.BLUETOOTH_ADVERTISE))) {
        FabricRuntime.bleDetail = "Bluetooth scan/advertise permission required"
        return
      }
      ...
      FabricRuntime.bleDetail = "BLE start failed: ${e.javaClass.simpleName}"
    }
  }

  private fun recordScan(result: ScanResult) {
    val id = payloadIdentity(result.scanRecord?.getManufacturerSpecificData(MANUFACTURER_ID)) ?: return
    ...
    while (true) {
      ...
      if (now - first.ms <= 8000) break else queue.pollFirst()
    }
  }
  ...
  private fun startNetworkThread(ctx: Context) {
    thread(name = "BodyFinderFabricV2", isDaemon = true) {
      try {
        ...
        try { socket.joinGroup(InetAddress.getByName(GROUP)) } catch (_: Throwable) {}
        ...
        while (FabricRuntime.running) {
          ...
          if (now >= nextSend) {
            ...
            try { socket.send(DatagramPacket(bytes, bytes.size, broadcastAddress, PORT)) } catch (_: Throwable) {}
            nextSend = now + 800
          }
          try {
