...
export type NativeApi = {
  ...
  updateControlPlaneJson(controlPlaneJson: string | null): boolean;
...
const EVIDENCE_SCHEMA='dev20.6-self-contained-json-evidence-v9';
...
function upgradeValidationContract(raw:string):string{try{const d=JSON.parse(raw);const v=d?.validation_run??d;if(v&&typeof v==='object'){const elapsed=Number(v.elapsed_ms??v.snapshot_elapsed_ms??0);v.acceptance_minimum_ms=ACCEPTANCE_MINIMUM_MS;v.acceptance_duration_eligible=elapsed>=ACCEPTANCE_MINIMUM_MS;v.short_diagnostic_run=elapsed>0&&elapsed<ACCEPTANCE_MINIMUM_MS;v.evidence_contract_version=EVIDENCE_SCHEMA;}if(Array.isArray(d?.completed_validation_runs_summary))for(const v of d.completed_validation_runs_summary){const elapsed=Number(v?.elapsed_ms??0);v.acceptance_minimum_ms=ACCEPTANCE_MINIMUM_MS;v.acceptance_duration_eligible=elapsed>=ACCEPTANCE_MINIMUM_MS;v.short_diagnostic_run=elapsed>0&&elapsed<ACCEPTANCE_MINIMUM_MS;}return JSON.stringify(d);}catch{return raw;}}
function upgradeExport(raw:string):string{try{const d=JSON.parse(raw);const preview=d?.human_presence_preview??d?.validation_truth?.authoritative_presence??d?.validation_run?.validation_truth?.authoritative_presence??null;const digest=preview?.canonical_digest??null;d.evidence_contract={...(d.evidence_contract??{}),schema:EVIDENCE_SCHEMA,screenshots_required:false,json_self_contained:true,required_external_input:'ground_truth_and_scenario_metadata_only_for_final_validator',diagnostic_source:'this JSON export',atomic_snapshot:true};d.acceptance_minimum_ms=ACCEPTANCE_MINIMUM_MS;d.human_localization_validated=false;d.rescue_use_validated=false;d.snapshot_consistency_digest=digest;d.snapshot_consistency_source=digest?'authoritative_presence.canonical_digest':'NO_AUTHORITATIVE_DECISION_AT_SNAPSHOT';if(d.validation_run){const elapsed=Number(d.validation_run.elapsed_ms??0);d.validation_run.acceptance_minimum_ms=ACCEPTANCE_MINIMUM_MS;d.validation_run.acceptance_duration_eligible=elapsed>=ACCEPTANCE_MINIMUM_MS;d.validation_run.short_diagnostic_run=elapsed>0&&elapsed<ACCEPTANCE_MINIMUM_MS;d.validation_run.evidence_contract_version=EVIDENCE_SCHEMA;}return JSON.stringify(d,null,2);}catch{return raw;}}
