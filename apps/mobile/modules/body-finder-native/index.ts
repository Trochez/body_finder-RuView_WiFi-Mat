...
function upgradeValidationContract(raw:string):string{try{const d=JSON.parse(raw);const v=d?.validation_run??d;if(v&&typeof v==='object'){const elapsed=Number(v.elapsed_ms??v.snapshot_elapsed_ms??0);v.acceptance_minimum_ms=ACCEPTANCE_MINIMUM_MS;v.acceptance_duration_eligible=elapsed>=ACCEPTANCE_MINIMUM_MS;v.short_diagnostic_run=elapsed>0&&elapsed<ACCEPTANCE_MINIMUM_MS;v.evidence_contract_version='dev20.3-self-contained-json-evidence-v6';}if(Array.isArray(d?.completed_validation_runs_summary))for(const v of d.completed_validation_runs_summary){const elapsed=Number(v?.elapsed_ms??0);v.acceptance_minimum_ms=ACCEPTANCE_MINIMUM_MS;v.acceptance_duration_eligible=elapsed>=ACCEPTANCE_MINIMUM_MS;v.short_diagnostic_run=elapsed>0&&elapsed<ACCEPTANCE_MINIMUM_MS;}return JSON.stringify(d);}catch{return raw;}}
function upgradeExport(raw:string):string{try{const d=JSON.parse(raw);d.evidence_contract={...(d.evidence_contract??{}),schema:'dev20.3-self-contained-json-evidence-v6',screenshots_required:false,json_self_contained:true,required_external_input:'ground_truth_labels_only_for_acceptance',diagnostic_source:'this JSON export'};d.acceptance_minimum_ms=ACCEPTANCE_MINIMUM_MS;d.human_localization_validated=false;d.rescue_use_validated=false;if(d.validation_run){const elapsed=Number(d.validation_run.elapsed_ms??0);d.validation_run.acceptance_minimum_ms=ACCEPTANCE_MINIMUM_MS;d.validation_run.acceptance_duration_eligible=elapsed>=ACCEPTANCE_MINIMUM_MS;d.validation_run.short_diagnostic_run=elapsed>0&&elapsed<ACCEPTANCE_MINIMUM_MS;d.validation_run.evidence_contract_version='dev20.3-self-contained-json-evidence-v6';}return JSON.stringify(d,null,2);}catch{return raw;}}
