...
function latestPerObserver(samples: RangeObservation[]) {
  const latest = new Map<string, RangeObservation>();
  for (const sample of samples) {
    const current = latest.get(sample.observer_node_id);
    if (!current || sample.monotonic_ns > current.monotonic_ns) latest.set(sample.observer_node_id, sample);
  }
  return [...latest.values()].sort((a, b) => a.observer_node_id.localeCompare(b.observer_node_id));
}
...
export function applyReciprocalFusion(input: Advertisement[]): {
  ...
  for (const [key, grouped] of groups) {
    ...
    const samples = latestPerObserver(grouped);
