...
function syncAuthoritativeCalibration(nodes:Advertisement[],coordinatorNodeId:string|null,localNodeId:string|null){
  ...
  if((p?.schema==='CalibrationMetaWireV2'||p?.schema==='CalibrationMetaV10')&&artifact?.calibration_hash===p?.hash&&authority.view&&p?.authority_digest===authority.view.authority_view_digest&&p?.session_id===authority.view.session_id&&p?.coordinator_id===coordinatorNodeId){
  ...
  }else if((p?.schema==='CalibrationMetaWireV2'||p?.schema==='CalibrationMetaV10')&&!artifact&&!cal.artifact){cal={...cal,state:'WAIT_COORDINATOR',coordinator:coordinatorNodeId,reason:'CALIBRATION_FINAL_V10_PENDING'}}
...
export function getControlPlanePublication(nodes:Advertisement[],coordinatorNodeId:string|null,localNodeId:string|null){
  ...
  const artifacts:any[]=[];if(coordinatorNodeId===localNodeId&&cm&&cal.artifact)artifacts.push({artifact_id:cm.a,artifact_type:'CALIBRATION_FINAL_V10',generation:cal.generation,priority:'CALIBRATION_FINAL',supersedes_artifact_id:null,payload:cal.artifact});const authority=getAuthorityControlPublication(nodes,localNodeId);
...
export function selectAuthoritativePresence(nodes:Advertisement[],coordinatorNodeId:string|null,localNodeId:string|null,local:PresenceEstimate):PresenceEstimate{
  ...
  const coordinator=nodes.find(n=>n.node_id===coordinatorNodeId);const dp=(coordinator?.control_plane as any)?.decision_meta_v10;
