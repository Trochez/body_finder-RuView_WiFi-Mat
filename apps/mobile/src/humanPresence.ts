...
function syncAuthoritativeCalibration(nodes:Advertisement[],coordinatorNodeId:string|null,localNodeId:string|null){
  ...
  if(p?.schema==='CalibrationMetaWireV3'&&localVerified?.complete===true&&artifact?.calibration_hash===p?.hash&&String(artifact?.calibration_id??'')===String(p?.id??'')&&authority.view&&p?.authority_digest===authority.view.authority_view_digest&&p?.session_id===authority.view.session_id&&p?.coordinator_id===coordinatorNodeId){
    ...
    if(sameOrNewer&&ordered){const receivedTopologyHash=assertCanonicalTopologyHash(String(p.topology_hash??''));cal={...cal,state:'READY',generation:incomingCal,coordinator:coordinatorNodeId,topologyHash:receivedTopologyHash,artifact,reason:'AUTHORITATIVE_CALIBRATION_LOCAL_RECEIVER_VERIFIED_PROMOTED',expectedCohort:authority.view.cohort.map((x:any)=>String(x.node_id)).sort(),publicationSequence:incomingSeq,lastAuthorityWallMs:now,coordinatorGeneration:incomingCg,authorityDigest:String(p.authority_digest)}}
