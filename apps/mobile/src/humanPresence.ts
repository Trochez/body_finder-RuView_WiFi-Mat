...
function syncAuthoritativeCalibration(nodes:Advertisement[],coordinatorNodeId:string|null,localNodeId:string|null){
  ...
  if(p?.schema==='CalibrationMetaWireV4'&&localVerified?.complete===true&&artifact?.calibration_hash===p?.hash&&String(artifact?.calibration_id??'')===String(p?.id??'')&&authority.view&&p?.authority_digest===authority.view.authority_view_digest&&p?.session_id===authority.view.session_id&&p?.coordinator_id===coordinatorNodeId){
  ...
  }else if(p?.schema==='CalibrationMetaWireV4'&&!artifact&&!cal.artifact){cal={...cal,state:'WAIT_COORDINATOR',coordinator:coordinatorNodeId,reason:'CALIBRATION_FINAL_V10_PENDING'}}
