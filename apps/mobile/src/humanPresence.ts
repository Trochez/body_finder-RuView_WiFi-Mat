...
export function getControlPlanePublication(nodes:Advertisement[],coordinatorNodeId:string|null,localNodeId:string|null){
  ...
  return{schema:'BodyFinderControlPlaneV8',session_id:sid,node_id:localNodeId,logical_membership_state:{expected_cohort:stableCohort(nodes),transport_liveness_state:transportStates(nodes)},calibration_ack_v6:ack,decision_publication_v8:decisionPublication,decision_ack_v8:decisionAck}
}
...
export function selectAuthoritativePresence(nodes:Advertisement[],coordinatorNodeId:string|null,localNodeId:string|null,local:PresenceEstimate):PresenceEstimate{
  ...
  if(dp&&dp.schema==='DecisionPublicationV8'&&dp.session_id===sid&&dp.coordinator_id===coordinatorNodeId&&Number(dp.coordinator_generation)===cal.coordinatorGeneration&&dp.detector_algorithm===DETECTOR_ALGORITHM&&dp.detector_parameter_hash===DETECTOR_PARAMETER_HASH&&dp.calibration_hash===cal.artifact?.calibration_hash&&dp.canonical_digest&&dp.decision_artifact?.canonical_digest===dp.canonical_digest&&dp.decision_artifact?.decision_id===dp.decision_id){
    const incomingSeq=Number(dp.decision_sequence??0),previous=latestDecisionBySession.get(sid);
    if(!previous||incomingSeq>=previous.sequence){const mirrored={...dp.decision_artifact,calibration_state:cal.artifact?'READY':dp.decision.calibration_state,source:'decision_control_plane_v8',decision_sequence:incomingSeq,decision_freshness_state:'FRESH'} as PresenceEstimate;latestDecisionBySession.set(sid,{decision:mirrored,receivedWallMs:Date.now(),sequence:incomingSeq});return mirrored}
  }
