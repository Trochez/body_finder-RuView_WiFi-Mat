...
import { DETECTOR_ALGORITHM, DETECTOR_PARAMETER_HASH, DETECTOR_V6 } from './detectorParameters';
...
type CachedDecision={decision:PresenceEstimate;receivedWallMs:number};

const HISTORY_MS=90_000;
const DECISION_CACHE_MS=6_000;
...
function updateMembership(nodes:Advertisement[]):Membership|null{
  ...
  if(m.coordinator&&!active.has(m.coordinator)){
    ...
    if(absentFor>=DETECTOR_V6.coordinatorFailoverGraceMs&&quorum.length>=2){
...
function confirmedMembershipChanges(nodes:Advertisement[]){
  ...
  return Object.entries(m.unexpectedSince).filter(([,since])=>now-since>=DETECTOR_V6.membershipChangeGraceMs).map(([id])=>id).sort();
}
...
function topology(links:Link[],cohort:string[]){const live=expectedLinks(links,cohort).filter(l=>l.samples.length&&Date.now()-l.samples.at(-1)!.receive_wall_ms<DETECTOR_V6.transportEvidenceFreshMs);const nodes=new Set(live.map(l=>l.observer_node_id));const baselines=new Set(live.map(l=>pair(l.observer_node_id,l.peer_node_id)));return{ok:cohort.length===3&&nodes.size>=3&&live.length>=6&&baselines.size>=3,fingerprint:cohortFingerprint(cohort),links:live,nodes:nodes.size,baselines:baselines.size}}
...
function syncAuthoritativeCalibration(nodes:Advertisement[],coordinatorNodeId:string|null,localNodeId:string|null){
  ...
  }else if(cal.artifact&&cal.lastAuthorityWallMs&&now-cal.lastAuthorityWallMs>DETECTOR_V6.authorityPublicationLeaseMs){cal={...cal,state:'STALE_AUTHORITY',reason:'AUTHORITY_PUBLICATION_LEASE_EXPIRED_CALIBRATION_PRESERVED'}}
}
...
function maybeFreeze(nodes:Advertisement[],coordinatorNodeId:string|null,localNodeId:string|null,links:Link[]){
  ...
  if(Date.now()-cal.started>DETECTOR_V6.calibrationTimeoutMs){cal={...cal,state:'INVALID',reason:'CALIBRATION_INSUFFICIENT_SUPPORT'};return}
  ...
  if(selected.some(l=>l.samples.length<DETECTOR_V6.calibrationMinSamplesPerLink)||overlap<DETECTOR_V6.calibrationMinOverlapMs)return;
  const session=currentSession(nodes),calibrationId=`cal-d206-${cal.generation}-${coordinatorNodeId?.slice(-8)??'none'}-${cal.started}`;
...
export function estimateHumanPresence(nodes:Advertisement[],mode:'coordinator'|'diagnostic'='diagnostic',coordinatorNodeId:string|null=null,localNodeId:string|null=null):PresenceEstimate{
  ...
  const now=Date.now(),start=Math.max(Number(cal.artifact.frozen_wall_ms)+250,now-DETECTOR_V6.observationWindowMs);
  const obs=expectedLinks(links,cal.expectedCohort).map(l=>({...l,samples:l.samples.filter(s=>s.receive_wall_ms>=start).slice(-240)}));
  const support=obs.map(l=>({link_id:l.link_id,actual_samples:l.samples.length,min_samples:DETECTOR_V6.observationMinSamplesPerLink,first_receive_wall_ms:l.samples[0]?.receive_wall_ms??null,last_receive_wall_ms:l.samples.at(-1)?.receive_wall_ms??null}));
  const insufficient=support.filter(s=>s.actual_samples<DETECTOR_V6.observationMinSamplesPerLink);
  if(obs.length!==6||insufficient.length)return fallback('OBSERVATION_SUPPORT_INCOMPLETE_DEADLINE_NOT_YET_ADMISSIBLE','READY',{observation_support:{window_ms:DETECTOR_V6.observationWindowMs,links:support,insufficient_links:insufficient.map(x=>x.link_id)},transport_liveness_state:transport});
...
export function selectAuthoritativePresence(nodes:Advertisement[],coordinatorNodeId:string|null,localNodeId:string|null,local:PresenceEstimate):PresenceEstimate{
  ...
  if(coordinatorNodeId===localNodeId){latestDecisionBySession.set(sid,{decision:local,receivedWallMs:Date.now()});return local}
