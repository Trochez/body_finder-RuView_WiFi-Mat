...
function publicationFrom(nodes:Advertisement[],coordinatorNodeId:string|null){const coordinator=nodes.find(n=>n.node_id===coordinatorNodeId);return (coordinator?.control_plane as any)?.calibration_publication_v9??null}
...
function syncAuthoritativeCalibration(nodes:Advertisement[],coordinatorNodeId:string|null,localNodeId:string|null){
  ...
  if(p?.schema==='CalibrationPublicationV9'&&p?.detector_parameter_hash===DETECTOR_PARAMETER_HASH&&artifact?.calibration_hash===p?.calibration_hash){
  ...
  }else if(p?.schema==='CalibrationPublicationV9'&&!artifact&&!cal.artifact){cal={...cal,state:'WAIT_COORDINATOR',coordinator:coordinatorNodeId,reason:'CALIBRATION_ARTIFACT_V8_PENDING'}}
...
function exactAck(node:Advertisement,id:string){
  ...
  const a=(node.control_plane as any)?.calibration_ack_v9;
  return Boolean(a&&a.schema==='CalibrationAckV9'&&a.node_id===id&&a.calibration_id===cal.artifact.calibration_id&&a.calibration_hash===cal.artifact.calibration_hash&&Number(a.calibration_generation)===cal.generation&&Number(a.coordinator_generation)===cal.coordinatorGeneration);
}
