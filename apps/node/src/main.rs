...
use body_finder_core::{
    elect_coordinator, estimate_from_rssi, CapabilityProbe, CapabilityState, NodeAdvertisement,
    NodePosition, FABRIC_PORT, PROTOCOL_VERSION,
};
use std::{
...
fn wifi_rssi() -> Option<f64> {
    ...
    for line in txt.lines().skip(2) {
        ...
        if parts.len() >= 4 {
            if let Ok(v) = parts[3].trim_end_matches('.').parse::<f64>() {
                return Some(v);
            }
        }
    }
    ...
    c.insert(
        ...
        CapabilityProbe {
            ...
            detail: if rssi.is_some() {
                "live OS Wi-Fi link metric read".into()
            } else {
                "no accessible Wi-Fi RSSI interface found".into()
            },
        },
    );
    let bt_ok = if cfg!(target_os = "linux") {
        output("bluetoothctl", &["show"]).is_some()
    } else {
        false
    };
    c.insert(
        ...
        CapabilityProbe {
            ...
            detail: if bt_ok {
                "BlueZ controller visible; active ranging not yet validated".into()
            } else {
                "BLE fabric adapter not active in this build".into()
            },
        },
    );
    c.insert(
...
fn main() -> Result<()> {
    ...
    eprintln!(
        "Body Finder node={} platform={} UDP={} baseline={:?} sigma={:?}",
        args.node_id, plat, FABRIC_PORT, baseline, baseline_sigma
    );
    loop {
        let rssi = wifi_rssi();
        ...
        let mut all: Vec<NodeAdvertisement> =
